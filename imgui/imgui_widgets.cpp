...
void AddRectMultiColor(const ImVec2& pMin, const ImVec2& pMax, ImU32 topColor, ImU32 bottomColor, ImU32 leftColor, ImU32 rightColor)
{
    ImVec2 verts[4];
    verts[0] = pMin;
    verts[1] = ImVec2(pMax.x, pMin.y);
    verts[2] = pMax;
    verts[3] = ImVec2(pMin.x, pMax.y);

    ImDrawList* drawList = ImGui::GetWindowDrawList();
    drawList->AddConvexPolyFilled(verts, 4, topColor);

    drawList->AddRectFilledMultiColor(pMin, ImVec2(pMin.x + 2, pMax.y), leftColor, leftColor, rightColor, rightColor);
    drawList->AddRectFilledMultiColor(ImVec2(pMax.x - 2, pMin.y), pMax, leftColor, leftColor, rightColor, rightColor);

    drawList->AddConvexPolyFilled(verts, 4, bottomColor);
}
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    const ImU32 col = ImColor((held && hovered) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered : ImColor(23, 9, 41, int(255 * GetStyle().Alpha)));
    RenderNavHighlight(bb, id);

    ImU32 color1 = IM_COL32(106, 51, 184, int(125)); // 106, 51, 184, int(125)
    ImU32 color2 = IM_COL32(45, 19, 82, int(55));

    window->DrawList->AddRectFilled(bb.Min, bb.Max, col, 4);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, col, 4);
    window->DrawList->AddRect(bb.Min, bb.Max, color1, 4);
    //AddRectMultiColor(bb.Min, bb.Max, ImColor(171, 92, 255, (int)(GetStyle().Alpha * 1.f)), ImColor(171, 92, 255, (int)(GetStyle().Alpha * 1.f)), ImColor(171, 92, 255, (int)(GetStyle().Alpha * 255.f)), ImColor(171, 92, 255, (int)(GetStyle().Alpha * 255.f)));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(23, 9, 41, int(255 * GetStyle().Alpha)), 4);
        AddRectMultiColor(frame_bb.Min, frame_bb.Max, ImColor(171, 92, 255, (int)(GetStyle().Alpha * 1.f)), ImColor(171, 92, 255, (int)(GetStyle().Alpha * 1.f)), ImColor(171, 92, 255, (int)(GetStyle().Alpha * 255.f)), ImColor(171, 92, 255, (int)(GetStyle().Alpha * 255.f)));
    }
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            ImU32 col = ImColor(255, 255, 255, (int)155);
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ...
    ImVec2 size(170.0f, ImFloor(GetTextLineHeightWithSpacing() * height_in_items_f + g.Style.FramePadding.y * 2.0f + 17));
